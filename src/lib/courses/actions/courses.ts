...
export async function createCourse(input: CourseInput): Promise<ActionResult> {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await logAudit({
      ...
      clubId,
      summary: `Added class ${courseLabel(course)} teaching ${level.name}, ${formatSlot(course)}`,
    }, tx);
  });
...
export async function updateCourse(id: string, input: CourseInput): Promise<ActionResult> {
  ...
  const result = await withCourseSeat(id, async (tx) => {
    ...
    if (data.levelId !== existing.levelId) {
      const open = await tx.enrolment.count({ where: { courseId: id, status: { in: ["ACTIVE", "WAITLISTED"] } } });
      if (open > 0) return fail(
        `${open} ${open === 1 ? "swimmer is" : "swimmers are"} enrolled or waitlisted at ${existing.level.name}. End those enrolments, or make a new class for the other level.`
      );
    }
    ...
    if (changes.length > 0) {
      await logAudit({
        ...
        clubId,
        summary:
          `Updated class ${courseLabel(course)} (${changes.join(", ")})` +
...
export async function setCourseArchived(id: string, archived: boolean): Promise<ActionResult> {
  ...
  const result = await withCourseSeat(id, async (tx) => {
    ...
    if (archived && existing._count.enrolments > 0) {
      ...
      return fail(
        `${n} ${n === 1 ? "swimmer is" : "swimmers are"} still enrolled or waitlisted. End or move ${n === 1 ? "that enrolment" : "those enrolments"} first.`
      );
    }
    ...
    await logAudit({
      ...
      clubId,
      summary: `${archived ? "Archived" : "Restored"} class ${courseLabel(existing)}`,
    }, tx);
    return ok();
  });
  if (!result.ok) return result;
