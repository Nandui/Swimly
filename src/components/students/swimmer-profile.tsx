...
export function SwimmerProfile({ student, enrolments, programmes, assessments, targets, history, access, initialTab, returnTo, instant }: SwimmerProfileProps) {
  ...
  return <article className={styles.profile} data-swimmer-profile>
    <a href={returnTo} className="inline-flex min-h-11 items-center gap-2 text-sm text-ui-muted-foreground hover:text-ui-foreground"><ArrowLeft className="size-4" aria-hidden="true" />Swimmers</a>
    <header className={styles.header}>
      <div className={styles.identity}><Avatar className={styles.avatar} aria-hidden="true"><AvatarFallback className="text-[inherit]">{student.firstName.slice(0, 1)}{student.lastName.slice(0, 1)}</AvatarFallback></Avatar><div className="min-w-0"><h1 className="text-3xl font-semibold tracking-tight [overflow-wrap:anywhere]">{name}</h1><p className="mt-2 text-sm text-ui-muted-foreground">{student.dateOfBirth ? `Age ${ageInYears(student.dateOfBirth, new Date(instant))} · ` : ""}{student.memberNumber ? `Member ${student.memberNumber} · ` : ""}Joined {formatDate(student.joinedOn)}</p>{student.status !== "ACTIVE" ? <Badge variant="secondary" data-tone={STUDENT_STATUS_META[student.status].color}>{STUDENT_STATUS_META[student.status].label}</Badge> : null}</div></div>
    ...
    <Tabs value={tab} onValueChange={value => navigate(value as Tab)} className="mt-3 gap-0">
      ...
      <div className={styles.body}>
        <div className="min-w-0">
          <TabsContent value="journey" className="m-0 space-y-5">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-semibold tracking-tight">Every chapter, from their first swim</h2><p className="mt-1 text-sm text-ui-muted-foreground">Classes, progress and activity across both sites.</p></div>
