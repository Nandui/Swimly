...
import { canMarkRegister, canTeachClass, needsTakeOver } from "@/lib/attendance/access";
...
import { classReturnDestination, instructorClassHref, type ClassWorkspace, type ClassQuery } from "@/lib/attendance/navigation";
...
export async function ClassSession({ id, params, workspace }: { id: string; params: ClassQuery; workspace: ClassWorkspace }) {
  ...
  }, params.from, course.id, workspace, params);
  ...
  const mayMark = !course.archivedAt && (workspace === "instructor" ? canTeachClass(access) : canMarkRegister(access));
  ...
  const stepHref = (next: Step, date = iso) => workspace === "instructor"
        ...
        <PageHeader
            ...
            step === "attendance" ? (
              <>
                <Button
                  label="Week before"
                  variant="secondary"
                  href={stepHref("attendance", shiftWeeks(iso, -1))}
          ...
          title={
            <HStack gap={2} vAlign="center" wrap="wrap">
              ...
              {cover && cover.coverById !== cover.instructorId ? <Tag color={ATTENDANCE_RECORD_META.covered.color}>{ATTENDANCE_RECORD_META.covered.label}</Tag> : null}
            </HStack>
          }
          description={
