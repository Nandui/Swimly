...
export function BillingList({ rows, canNotify, notified }: { rows: BillingCancellation[]; canNotify: boolean; notified: boolean }) {
  if (!rows.length) return <div className="rounded-ui-lg border border-dashed border-ui-border p-8 text-center"><h2 className="font-semibold">{notified ? "No billing notifications recorded" : "No cancellations awaiting billing"}</h2><p className="mt-1 text-sm text-ui-muted-foreground">{notified ? "Completed handoffs will stay here for reference." : "Cancelled class sessions will appear here with their affected swimmers."}</p></div>;
