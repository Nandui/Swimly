-- Dedicated Docs database. Staff identities are validated by Turnfin, not copied here.
CREATE SCHEMA IF NOT EXISTS turnfin_docs;
...
CREATE TABLE member_profiles (id text PRIMARY KEY, facility_ids text[] NOT NULL DEFAULT '{}', team_ids text[] NOT NULL DEFAULT '{}');
...
CREATE TABLE IF NOT EXISTS documents (
  id text PRIMARY KEY, created_at timestamptz NOT NULL DEFAULT now(), created_by text NOT NULL,
...
CREATE TABLE IF NOT EXISTS snapshots (
  ...
  author_id text NOT NULL, approver_id text, submission_id text,
...
CREATE TABLE IF NOT EXISTS drafts (
  ...
  change_summary text NOT NULL DEFAULT '', approver_id text,
  submission_id text REFERENCES snapshots, feedback text NOT NULL DEFAULT '',
  lease_owner text, lease_session text, lease_until timestamptz,
  updated_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS reviews (
  id text PRIMARY KEY, submission_id text UNIQUE NOT NULL REFERENCES snapshots,
  reviewer_id text NOT NULL, decision text NOT NULL CHECK(decision IN ('approved','changes_requested')),
...
CREATE TABLE IF NOT EXISTS requirements (
  ...
  member_id text NOT NULL, status text NOT NULL DEFAULT 'outstanding' CHECK(status IN ('outstanding','completed','cancelled')),
  due_date date, UNIQUE(version_id, member_id)
);
CREATE TABLE IF NOT EXISTS acknowledgements (id text PRIMARY KEY, version_id text NOT NULL REFERENCES snapshots, member_id text NOT NULL, created_at timestamptz NOT NULL DEFAULT now(), UNIQUE(version_id,member_id));
CREATE TABLE IF NOT EXISTS audit_events (id text PRIMARY KEY, actor_id text NOT NULL, document_id text REFERENCES documents, action text NOT NULL, detail text NOT NULL, created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS attachments (id text PRIMARY KEY, document_id text NOT NULL REFERENCES documents, name text NOT NULL, mime text NOT NULL, size integer NOT NULL CHECK(size>0), storage_key text UNIQUE NOT NULL, created_by text NOT NULL);
...
CREATE TABLE storage_imports (id text PRIMARY KEY, source text NOT NULL, counts jsonb NOT NULL, imported_at timestamptz NOT NULL DEFAULT now());
ALTER TABLE storage_imports ENABLE ROW LEVEL SECURITY;
RESET search_path;
