...
export async function buildScreenshots() {
  ...
  await fs.mkdir(path.join(output,'brand'),{recursive:true});
  await esbuild.build({entryPoints:['scripts/help-screenshots/fixture.jsx'],bundle:true,outdir:output,jsx:'automatic',platform:'browser',format:'iife',define:{'process.env.NODE_ENV':'"production"','process.env':'{}'},loader:{'.woff':'file','.woff2':'file'},plugins:[fixture],minify:true});
  ...
  await fs.copyFile('public/brand/app-logo.png',path.join(output,'brand/app-logo.png'));
  await fs.writeFile(path.join(output,'index.html'),'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Help screenshot examples</title><link rel="icon" href="data:,"><link rel="stylesheet" href="/app.css"><link rel="stylesheet" href="/fixture.css"></head><body><div id="root"></div><script src="/fixture.js"></script></body></html>');
}
